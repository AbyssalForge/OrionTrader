...
def _(mo):
 ...
 ## Points Clés pour le Trading ML
...
def _(corr_matrix, mo, pd):
 ...
 # MARIMO: Utiliser des noms uniques pour éviter conflits avec autres cellules
 corr_pairs = []
...
def _(mo):
 ...
 ### Règle Critique : Éviter le Data Leakage
...
def _(df_raw, pd):
 ...
 # CORRECTION CRITIQUE: shift(1) sur TOUTES les features techniques
...
def _(df_fe, horizon, volatility_multiplier):
 ...
 # CRITIQUE: shift(1) pour éviter le leakage (ne pas inclure le return courant)
 ...
 # CORRECTION: Remplacer apply() par opérations vectorisées
 ...
 ** CHANGEMENT IMPORTANT (Recommandation Expert)**
...
def _(corr_threshold, df_labeled, np, pd):
 # PROTECTION: Vérifier que df_labeled n'est pas vide
 ...
 # CORRECTION: Inclure aussi float64 via np.number pour plus de robustesse
 ...
 print(" ATTENTION: Aucune feature numérique trouvée!")
 ...
 # CHANGEMENT: On garde TOUTES les features (pas de filtrage)
...
def _(mo):
 ...
 #### 4 Split Train/Test Chronologique (80/20)

 ```
 CRITIQUE: PAS de shuffle aléatoire en time-series !
...
def _(LabelEncoder, StandardScaler, df_labeled, features_to_keep, np, pd):
 ...
 # CORRECTION: Utiliser np.number pour une détection plus robuste des types numériques
 ...
 # CORRECTION: Exclure 'close' des features pour éviter duplication
 ...
 # CORRECTION CRITIQUE: Exclure les colonnes avec trop de NaN (> 50%)
 ...
 print(f"\n Colonnes exclues (>{MAX_NAN_PERCENT}% NaN):")
 ...
 # CORRECTION: S'assurer que 'close' n'est pas dupliquée
 ...
 # DIAGNOSTIC: Vérifier que feature_cols n'est pas vide
 has_error = False
 ...
 # DIAGNOSTIC: Vérifier que X n'est pas vide
 ...
 # CHANGEMENT IMPORTANT: PAS de scaling pour LightGBM
 ...
 print("\n Le pipeline ne peut pas continuer avec des données vides.")
...
def _(
 ...
 # PROTECTION: Vérifier que les données ne sont pas vides
 # Note: Pas de early return dans Marimo - utiliser if/else
 ...
 # Split temporel pur (SANS stratify, SANS shuffle)
...
def _(
 ...
 # MARIMO: Utiliser nom unique pour éviter conflit
 y_pred_backtest = []
 ...
 # CORRECTION CRITIQUE: Aligner le return avec l'horizon du label
 ...
 print(f" Backtest aligné sur horizon={horizon.value} bougies ({horizon.value * 15}min)")
 ...
 if neutral_pct < 50:
 print(f" WARNING: Trop agressif! Recommandé: 55-70% NEUTRAL")
 elif neutral_pct > 70:
 print(f" WARNING: Trop conservateur! Baisser les seuils")
 else:
...
def _(X_test_scaled, best_model_optimized, pd, shap):
 ...
 print(f" Format SHAP inattendu, utilisation de valeurs par défaut")
 ...
 print(f" Ajustement: shap arrays ont {len(shap_short)} éléments, features en a {n_features}")
...
def _(
 ...
 'Class': class_name,
 'ECE': ece,
 'Status': ' Bien calibré' if ece < 0.05 else ' Mal calibré'
 })
...
def _(
 ...
 print("⏭ Pas de calibration appliquée")
 else:
...
def _(df_backtest, horizon, np, pd, proba_test_calibrated, calibration_applied):
 ...
 # CORRECTION: Gérer le cas où aucune combinaison n'a assez de trades
 if len(grid_df) > 0:
 ...
 print("\n Aucune combinaison n'a généré assez de trades (>10)")
...
def _(grid_df, mo):
 ...
 mo.md(" Aucun résultat à afficher - pas assez de trades")
 return
...
def _(grid_df, mo, plt, sns):
 ...
 # CORRECTION: Vérifier que grid_df n'est pas vide
 if len(grid_df) == 0:
 mo.md(" Pas de données pour la heatmap")
 else:
...
def _(
 ...
 try:
 # CORRECTION 1: Fermer tout run actif (sécurité Marimo)
 if mlflow.active_run():
 print(" Run MLflow actif détecté → fermeture")
 mlflow.end_run()
 ...
 print(f" Erreur matrice confusion: {e}")
 ...
 print(f" SHAP values ne sont pas au bon format")
 ...
 print(f" Alias 'production' géré par le DAG CD (cd_model_pipeline)")
 except Exception as e2:
 print(f" Enregistrement Model Registry échoué: {e2}")
 ...
 print(f" Méthode native complètement échouée: {e3}")
 except Exception as e:
